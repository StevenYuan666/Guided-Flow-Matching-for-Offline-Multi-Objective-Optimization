...
class LanguageModel(nn.Module):
    ...
    def sample_tgt_tok_idxs(self, tgt_tok_logits, tgt_mask, temp=1.0):
...
class LengthTransform(nn.Module):
    ...
    def forward(
        ...
        src_arange = (
            src_arange.expand(batch_size, -1).unsqueeze(-1).float()
        )  # (batch_size, num_src_tokens, 1)

        tgt_arange = torch.arange(tgt_lens.max()).to(src_mask.device)
        tgt_arange = (
            tgt_arange.expand(batch_size, -1).unsqueeze(-2).float()
        )  # (batch_size, 1, num_tgt_tokens)
        ...
        tgt_token_features = (
            weights * tgt_token_features
        )  # (batch_size, num_src_tokens, num_tgt_tokens, embed_dim)
