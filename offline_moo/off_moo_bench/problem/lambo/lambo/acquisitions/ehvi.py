...
class EHVI(object):
    def __init__(
        ...
        **kwargs
        ...
        self.ref_point = (
            infer_reference_point(known_targets) if ref_point is None else ref_point
        )
        sampler = IIDNormalSampler(num_samples=num_samples)
        partitioning = NondominatedPartitioning(
            ref_point=self.ref_point, Y=known_targets
        )
        acq_kwargs = dict(
            model=surrogate,
    ...
    def __call__(self, candidates):
        acq_vals = torch.cat(batched_call(self.acq_fn, candidates, batch_size=1))
        return acq_vals
