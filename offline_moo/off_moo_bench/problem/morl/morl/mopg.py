    ...
    with torch.no_grad():
        for eval_id in range(args.eval_num):
            ...
            while not done:
                if args.ob_rms:
                    ob = np.clip(
                        (ob - ob_rms.mean) / np.sqrt(ob_rms.var + 1e-8), -10.0, 10.0
                    )
                _, action, _, _ = policy.act(
                    torch.Tensor(ob).double().unsqueeze(0),
                    None,
                    None,
                    deterministic=True,
                )
    ...
    for j in range(start_iter, final_iter):
        ...
        if args.use_linear_lr_decay:
            ...
            utils.update_linear_schedule(
                agent.optimizer, j * args.lr_decay_ratio, total_num_updates, args.lr
            )
        ...
        for step in range(args.num_steps):
            ...
            for idx, info in enumerate(infos):
                ...
                episode_obj[idx] = (
                    info["obj_raw"]
                    if episode_obj[idx] is None
                    else episode_obj[idx] + info["obj_raw"]
                )
        ...
        value_loss, action_loss, dist_entropy = agent.update(
            rollouts, scalarization, obj_rms_var
        )
        ...
        env_params = {}
        env_params["ob_rms"] = (
            deepcopy(envs.ob_rms) if envs.ob_rms is not None else None
        )
        env_params["ret_rms"] = (
            deepcopy(envs.ret_rms) if envs.ret_rms is not None else None
        )
        env_params["obj_rms"] = (
            deepcopy(envs.obj_rms) if envs.obj_rms is not None else None
        )
        ...
        if (
            args.rl_log_interval > 0
            and (j + 1) % args.rl_log_interval == 0
            ...
            if task_id == 0:
                ...
                print(
                    "[RL] Updates {}, num timesteps {}, FPS {}, time {:.2f} seconds".format(
                        j + 1,
                        total_num_steps,
