...
class KfacOptimizer:
        ...
        def searchFactors(gradient, graph):
            ...
            if "AddN" in bpropOp_name:
                ...
                assert len(np.unique(op_names)) == 1, (
                    gradient.name + " is shared among different computation OPs"
                )

                bTensors = reduce(
                    lambda x, y: x + y, [item["bpropFactors"] for item in factors]
                )
                ...
                if fpropOp.op_def.name in KFAC_OPS:
                    ...
                    ###
                    bTensor = [
                        i for i in bpropOp.inputs if "gradientsSampled" in i.name
                    ][-1]
                    ...
                    if fpropOp.op_def.name == "BiasAdd":
                        ...
                        fTensors.append(
                            [i for i in fpropOp.inputs if param.op.name not in i.name][
                                0
                            ]
                        )
                    ...
                    bInputsList = [
                        i
                        for i in bpropOp.inputs[0].op.inputs
                        if "gradientsSampled" in i.name
                        if "Shape" not in i.name
                    ]
                    if len(bInputsList) > 0:
                        ...
                        if (
                            ...
                            and bTensor.get_shape()[0].value == None
                        ):
                            bTensor.set_shape(bTensorShape)
                        bTensors.append(bTensor)
            ...
            return {
                "opName": fpropOp_name,
                "op": fpropOp,
                "fpropFactors": fTensors,
                "bpropFactors": bTensors,
            }
                ...
                for item in varlist:
                    if len(factorTensors[item]["bpropFactors"]) > 0:
                        if (
                            ...
                            factorTensors[param]["bpropFactors"] = factorTensors[item][
                                "bpropFactors"
                            ]
                ...
                if len(factorTensors[param][key]) > 0:
                    if (key + "_concat") not in factorTensors[param]:
                        ...
                        with tf.name_scope(name_scope):
                            factorTensors[param][key + "_concat"] = tf.concat(
                                factorTensors[param][key], 0
                            )
                else:
                    factorTensors[param][key + "_concat"] = None
                for j, param2 in enumerate(varlist[(i + 1) :]):
                    if (len(factorTensors[param][key]) > 0) and (
                        set(factorTensors[param2][key])
                        == set(factorTensors[param][key])
                    ):
                        factorTensors[param2][key] = factorTensors[param][key]
                        factorTensors[param2][key + "_concat"] = factorTensors[param][
                            key + "_concat"
                        ]
        ########
                ...
                for var in varlist:
                    ...
                    if fpropFactor is not None:
                        if fpropFactor not in tmpStatsCache:
                            ...
                            self.stats[var]["fprop_concat_stats"] = tmpStatsCache[
                                fpropFactor
                            ]
                    ...
                    if bpropFactor is not None:
                        ...
                        if not (
                            ...
                            if bpropFactor not in tmpStatsCache:
                                ...
                                self.stats[var]["bprop_concat_stats"] = tmpStatsCache[
                                    bpropFactor
                                ]
        ...
        for var in varlist:
            ...
            for stats_var in fpropStats_vars:
                ...
                if stats_var not in statsUpdates_cache:
                    ...
                    if (
                        stats_var_dim == (fpropFactor_size + 1)
                        ...
                        if opType == "Conv2D" and not self._approxT2:
                            ...
                            fpropFactor = tf.concat(
                                [
                                    fpropFactor,
                                    tf.ones([tf.shape(fpropFactor)[0], 1]) / Oh / Ow,
                                ],
                                1,
                            )
                        else:
                    ...
                    # divided by B
                    cov = tf.matmul(
                        fpropFactor, fpropFactor, transpose_a=True
            ...
            for stats_var in bpropStats_vars:
                ...
                if stats_var not in statsUpdates_cache:
                    ...
                    cov_b = tf.matmul(
                        bpropFactor, bpropFactor, transpose_a=True
                    ) / tf.to_float(tf.shape(bpropFactor)[0])
                ...
                return tf.cond(
                    ...
                    lambda: tf.group(
                        *self._apply_stats(
                            statsUpdates,
                            accumulate=True,
                            accumulateCoeff=1.0 / self._stats_accum_iter,
                        )
                    ),
                    tf.no_op,
                )
            ...
            if (len(fpropFactoredFishers) + len(bpropFactoredFishers)) > 0:
                ...
                if self._factored_damping:
                    ...
                    if len(eigVals) == 1:
                        ...
                        damping = tf.pow(
                            self._epsilon + weightDecayCoeff, 1.0 / num_factors
                        )
            ...
            with tf.control_dependencies(dependency_list):
                ...
                if not self._async:
                    ...
                    updateFactorOps = tf.cond(
                        ...
                        lambda: tf.group(
                            *self.applyStatsEigen(self.computeStatsEigen())
                        ),
                        no_op_wrapper,
                    )
                else:
    ...
    def apply_gradients(self, grads):
        ...
        return (
            tf.cond(
                tf.greater(self.sgd_step, self._cold_iter), warmKFACstart, coldSGDstart
            ),
            qr,
        )
