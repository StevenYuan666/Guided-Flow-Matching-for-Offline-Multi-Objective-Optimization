...
class FlowMatching(nn.Module):
    ...
    def gfmo_sample(
        ...
        """
        classifers: a list of classifiers, each is a function that takes x
        and returns the score on the i-th objective
        we use batch_size to denote the number of samples needed to generate,
        use num_solutions to denote the number of solutions we keep by using non-dominated sorting,
        use D to denote the dimension of the samples
        use M to denote the number of objectives, which is equal to the number of classifiers

        T: the number of steps to generate the samples
        O: the number of diverse samples to generate at each step
        K: the number of samples to include in the neighborhood
        ...
        m_closest_angles = FlowMatching.calculate_angles(
            ...
            objectives_weights.repeat_interleave(M, dim=0),  # shape: (batch_size, 1, len(classifiers))
        )
        ...
        ts, delta_t = FlowMatching.get_ts_and_delta_t(T, t_threshold=t_threshold, adaptive=adaptive)
        ...
        with tqdm(total=T, desc="Conditional Sampling", unit="step") as pbar:
            ...
            for t in ts[1:]:
                ...
                if t < t_threshold:
                    if need_repair:
                        x_t = self.repair_boundary(x_t, t, xl[0].unsqueeze(0).repeat(batch_size, 1), xu[0].unsqueeze(0).repeat(batch_size, 1))
                    pbar.update(1)
                    continue
        ...
        assert len(temp_pareto_set) >= num_solutions, "Error: The number of solutions in the pareto set is less than the number of solutions we want to keep"
        ...
        if task.is_discrete:
            temp_pareto_set = temp_pareto_set.reshape(temp_pareto_set.shape[0], task.x.shape[1], -1)
