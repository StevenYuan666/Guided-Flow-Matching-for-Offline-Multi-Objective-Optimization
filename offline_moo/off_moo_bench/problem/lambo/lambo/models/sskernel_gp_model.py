...
class SSKExactGP(SingleTaskExactGP):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        BaseGPSurrogate.__init__(
            self, tokenizer=tokenizer, encoder=encoder, *args, **kwargs
        )
    ...
    def prepare_match_features(self, all_preprocessed_strs):
        ...
        matches = valid_tok_idxs.eq(
            all_preprocessed_strs
        )  # (len(alphabet), *batch_shape, num_tokens)
        permute_dims = list(range(1, num_input_dims + 1))
        return matches.permute(*permute_dims, 0).to(
            self.device
        )  # (*batch_shape, num_tokens, len(alphabet))
    ...
    def get_features(self, seq_array, batch_size=None, transform=True):
        ...
        if seq_array.ndim == 1:
            if isinstance(seq_array, np.ndarray):
                tokens = [torch.tensor(self.tokenizer.encode(x)) for x in seq_array]
                all_preprocessed_strs = padding_collate_fn(
                    tokens, self.tokenizer.padding_idx
                )
                if all_preprocessed_strs.shape[-1] > self.str_max_len:
                    all_preprocessed_strs = all_preprocessed_strs[
                        ..., : self.str_max_len
                    ]
                elif all_preprocessed_strs.shape[-1] < self.str_max_len:
                    padding = torch.zeros(
                        all_preprocessed_strs.shape[-2],
                        self.str_max_len - all_preprocessed_strs.shape[-1],
                    ).to(all_preprocessed_strs)
                    all_preprocessed_strs = torch.cat(
                        (all_preprocessed_strs, padding), -1
                    )
            else:
    ...
    def forward(self, inputs):
        ...
        mean = self.mean_module(
            torch.empty(features.shape[:-2], device=features.device)
        )
    ...
    def __call__(self, *args, **kwds):
        ...
        if self.training:
            ...
            if self.prediction_cache is None:
                with torch.no_grad():
                    self.train_targets = self.train_targets.to(
                        list(self.covar_module.parameters())[0]
                    )

                    # need to manually put the posterior together
                    train_train_covar = self.covar_module(train_features)
                    train_mean = self.mean_module(
                        torch.empty(
                            train_features.shape[:-2], device=train_features.device
                        )
                    )
                    train_with_likelihood = self.likelihood(
                        MultivariateNormal(train_mean, train_train_covar)
                    )
            ...
            test_train_covar = self.covar_module(
            ...
            if mean_cache.shape[:-2] != test_train_covar.shape[:-2]:
                ...
                if mean_cache.ndim < test_train_covar.ndim:
                    ...
                    covar_cache = covar_cache.repeat(
                        1, test_train_covar.shape[-3], 1, 1
                    )
            ...
            test_mean = self.mean_module(
                torch.empty(*test_features.shape[:-2], device=train_features.device)
            )
            if had_double_batch:
                # TODO: why does this not cut across the 2nd q batch dim
                test_mean = test_mean.unsqueeze(-1).repeat(
                    *[1] * test_mean.ndim, test_features.shape[-3]
                )
            ...
            pred_covar = test_test_covar - second_term.matmul(
                second_term.transpose(-1, -2)
            )
    ...
    def posterior(
        ...
        **kwargs,
        ...
        if (
            (batch_size is None and self.eval_bs is None)
            or X.shape[-1] < self.eval_bs
            or X.shape[-1] < batch_size
        ):
        ...
        concatenated_mean = torch.cat(
            [dist.mean.squeeze() for dist in dist_list], dim=-1
        )
        # form the non-square block diagonal matrix
        if concatenated_mean.ndim > 1:
            concat_covar = torch.zeros(
                *concatenated_mean.shape[:-1],
                concatenated_mean.shape[-1],
                concatenated_mean.shape[-1],
                device=concatenated_mean.device,
                dtype=concatenated_mean.dtype,
            )
        else:
            concat_covar = torch.zeros(
                concatenated_mean.shape[-1],
        ...
        dist_type = (
            MultivariateNormal
            if isinstance(dist_list[0], MultivariateNormal)
            else MultitaskMultivariateNormal
        )
    ...
    def fit(
        ...
        Y_test,
        reset=False,
        log_prefix="single_task_string_kernel",
        **kwargs,
    ):
        if reset:
