    ...
    for i in range(len(opt_graph.objs)):
        ...
        if np.all(diff < np.abs(opt_graph.objs[index]) * threshold):
            for next_index in opt_graph.succ[i]:
                ...
                weights_data.append(
                    opt_graph.weights[next_index]
                    / np.sum(opt_graph.weights[next_index])
                )
...
def predict_hyperbolic(args, opt_graph, index, test_weights):
    ...
    while True:
        objs_data, weights_data, delta_objs_data = collect_nearest_data(
            opt_graph, index, threshold
        )
        cnt_data = 0
    ...
    def jac(params, x, y):
        ...
        J[1] = (
            A * (x - b) * (2.0 * np.exp(a * (x - b))) / ((np.exp(a * (x - b)) + 1) ** 2)
        ) * w

        # df_db = A(-a)(2exp(a(x-b)))/(exp(a(x-b)) + 1)^2
        J[2] = (
            A * (-a) * (2.0 * np.exp(a * (x - b))) / ((np.exp(a * (x - b)) + 1) ** 2)
        ) * w
...
class Population:
    def __init__(self, args):
        ...
        generate_weights_batch_dfs(
            ...
            0.0,
            1.0,
            1.0 / (args.pbuffer_num - 1),
            [],
            self.pbuffer_vec,
        )
        for i in range(len(self.pbuffer_vec)):
            self.pbuffer_vec[i] = self.pbuffer_vec[i] / np.linalg.norm(
                self.pbuffer_vec[i]
            )
        self.pbuffer_num = len(self.pbuffer_vec)
        self.pbuffers = [
            [] for _ in range(self.pbuffer_num)
        ]  # store the sample indices in each pbuffer
        self.pbuffer_dist = [
            [] for _ in range(self.pbuffer_num)
        ]  # store the sample distance in each pbuffer
    ...
    def insert_pbuffer(self, index, objs, enforce):
        ...
        if enforce:
            ...
            if inserted and len(self.pbuffers[buffer_id]) > self.pbuffer_size:
                ...
                self.pbuffer_dist[buffer_id] = self.pbuffer_dist[buffer_id][
                    : self.pbuffer_size
                ]
    ...
    def evaluate_hv(self, candidates, mask, virtual_ep_objs_batch):
        ...
        for i in range(len(candidates)):
            if mask[i]:
                new_ep_objs_batch = update_ep(
                    virtual_ep_objs_batch, candidates[i]["prediction"]
                )
    ...
    def evaluate_hypervolume_sparsity_parallel(
        self, args, candidates, mask, virtual_ep_objs_batch
    ):
        hv = [0.0 for _ in range(len(candidates))]
        sparsity = [0.0 for _ in range(len(candidates))]
        processes = []
        ...
        for i in range(len(candidates)):
            if mask[i]:
                p = Process(
                    target=update_ep_and_compute_hypervolume_sparsity,
                    args=(i, virtual_ep_objs_batch, candidates[i]["prediction"], queue),
                )
                p.start()
                processes.append(p)
    ...
    def prediction_guided_selection(
        self, args, iteration, ep, opt_graph, scalarization_template
    ):
        N = args.num_tasks  # number of (sample, weight) to be selected
        num_weights = (
            args.num_weight_candidates
        )  # for each sample, we have num_weights optimization directions to be candidates, they distribute evenly around the last weight direction
        ...
        for sample in self.sample_batch:
            ...
            for i in range(len(weight_indices)):
                if (
                    len(test_weights) >= num_weights
                ):  # if enough weights have been selected, then stop
                    break
                ...
                angle = np.arccos(
                    np.clip(
                        np.dot(weight_center, weight)
                        / np.linalg.norm(weight_center)
                ...
                if (
                    angle < np.pi / 4.0
                ):  # check if weight is close to the previous weight
                    duplicated = False
                    for succ in opt_graph.succ[sample.optgraph_id]:
                        w = deepcopy(opt_graph.weights[succ])
                        w = w / np.sum(w)
            ...
            if len(test_weights) > 0:
                results = predict_hyperbolic(
                    args, opt_graph, sample.optgraph_id, test_weights
                )
                for i in range(len(test_weights)):
                    candidates.append(
                        {
        ...
        for _ in range(N):
            hv, sparsity = self.evaluate_hypervolume_sparsity_parallel(
                args, candidates, mask, virtual_ep_objs_batch
            )
            ...
            scalarization.update_weights(
                candidates[best_id]["weight"] / np.sum(candidates[best_id]["weight"])
            )
            ...
            virtual_ep_objs_batch = update_ep(
                virtual_ep_objs_batch, predicted_new_objs[0]
            )
