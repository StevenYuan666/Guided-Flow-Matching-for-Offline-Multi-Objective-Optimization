...
class LaMBO(object):
    def __init__(
        ...
        **kwargs,
        ...
        self.bb_task = hydra.utils.instantiate(
            bb_task, tokenizer=tokenizer, candidate_pool=[]
        )
    ...
    def optimize(
        ...
        for round_idx in range(1, self.num_rounds + 1):
            ...
            if (
                self.concentrate_pool > 0 and round_idx % self.concentrate_pool == 0
            ) or self.latent_init == "perturb_pareto":
            ...
            X_test, Y_test = self.test_split.inputs, tgt_transform(
                self.test_split.targets
            )
            ...
            best_entry = {
                key.split("/")[-1]: val for key, val in records[best_idx].items()
            }
            ...
            for gen_idx in range(self.num_gens):
                ...
                base_idxs = np.random.choice(
                    np.arange(weights.shape[0]), batch_size, p=weights, replace=True
                )
                base_candidates = self.active_candidates[base_idxs]
                base_seqs = np.array(
                    [cand.mutant_residue_seq for cand in base_candidates]
                )
                ...
                if self.position_sampler == "entropy_method":
                    mask_idxs = self.sample_mutation_window(
                        window_mask_idxs, window_entropy
                    )
                ...
                with torch.no_grad():
                    ...
                    if self.latent_init == "perturb_pareto":
                        opt_features, src_mask = self.encoder.get_token_features(
                            src_tok_idxs
                        )
                        ...
                        np.put_along_axis(
                            src_tok_idxs,
                            mask_idxs,
                            self.encoder.tokenizer.masking_idx,
                            axis=1,
                        )
                        src_tok_features, src_mask = self.encoder.get_token_features(
                            src_tok_idxs
                        )
                        opt_features = np.take_along_axis(
                            src_tok_features, mask_idxs[..., None], axis=1
                        )
                    else:
                        raise ValueError
                    ...
                    opt_params = torch.empty(
                        *opt_features.shape,
                        requires_grad=self.optimize_latent,
                ...
                optimizer = torch.optim.Adam(
                    params=[opt_params], lr=self.lr, betas=(0.0, 1e-2)
                )
                ...
                for step_idx in range(self.num_opt_steps):
                    ...
                    if self.encoder_obj == "lanmt":
                        lat_tok_features, pooled_features = self.encoder.pool_features(
                            opt_params, src_mask
                        )
                        ...
                        tgt_tok_idxs, logit_entropy = self.encoder.sample_tgt_tok_idxs(
                            tgt_tok_logits, tgt_mask, temp=1.0
                        )
                    elif self.encoder_obj == "mlm":
                        current_features = src_tok_features.clone()
                        np.put_along_axis(
                            current_features, mask_idxs[..., None], opt_params, axis=1
                        )
                        lat_tok_features, pooled_features = self.encoder.pool_features(
                            current_features, src_mask
                        )
                        tgt_tok_logits, tgt_mask = self.encoder.logits_from_features(
                        ...
                        new_tok_idxs = np.take_along_axis(
                            new_tok_idxs, mask_idxs, axis=1
                        )
                        tgt_tok_idxs = src_tok_idxs.clone()
                        np.put_along_axis(tgt_tok_idxs, mask_idxs, new_tok_idxs, axis=1)
                        logit_entropy = np.take_along_axis(
                            logit_entropy, mask_idxs, axis=1
                        )
                    else:
                        raise ValueError
                    ...
                    loss = (
                        -lat_acq_vals.mean()
                        + self.entropy_penalty * logit_entropy.mean()
                    )
            ...
            base_seqs = np.array(
                [b_cand.mutant_residue_seq for b_cand in base_candidates]
            )
            ...
            assert (
                base_seqs.shape[0] == new_seqs.shape[0]
                and new_seqs.shape[0] == new_targets.shape[0]
            )
            for b_cand, n_cand, f_val in zip(
                base_candidates, new_candidates, new_targets
            ):
            ...
            pareto_seq_history = safe_np_cat(
                [pareto_seq_history, pareto_seqs[par_is_new]]
            )
    ...
    def sample_mutation_window(self, window_mask_idxs, window_entropy, temp=1.0):
        # selected_features = []
        selected_mask_idxs = []
