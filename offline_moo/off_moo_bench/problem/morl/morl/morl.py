...
def run(args):
    ...
    while iteration < total_num_updates:
        print(f"running iteration {iteration} out of {total_num_updates}")
        if episode == 0:
            print_info(
                "\n------------------------------- Warm-up Stage -------------------------------"
            )
            ...
            print_info(
                "\n-------------------- Evolutionary Stage: Generation {:3} --------------------".format(
                    episode
                )
            )
        ...
        if args.selection_method == "moead":
            ...
            scalarization_batch = []
            delta_ratio = (
                iteration + rl_num_updates + args.update_iter - args.warmup_iter
            ) / (total_num_updates - args.warmup_iter)
            ...
            for i in np.arange(
                ...
                w = np.clip(
                    i + delta_ratio * args.delta_weight,
                    args.min_weight,
                    args.max_weight,
                )
    ...
    data_dir = os.path.join(base_dir, "data", args.env_name)
    if not os.path.exists(data_dir):
        os.mkdir(data_dir)
    x_file = os.path.join(data_dir, f"{args.env_name}-x.npy")
    ...
    for i, sample in enumerate(ep.sample_batch):
        ...
        if not os.path.exists(params_shapes_path):
            params_shapes = [
                p.cpu().detach().numpy().shape for p in actor_critic.parameters()
            ]
            with open(params_shapes_path, "wb+") as f:
                pickle.dump(params_shapes, f)

        flat_params = np.array([])
        for p in sample.actor_critic.parameters():
        ...
        if x is None:
            ...
            x = np.concatenate((x, flat_params.reshape(1, -1)), axis=0)

        print(x.shape)
        np.save(file=x_file, arr=x)
        del x

        with open(
            os.path.join(args.save_dir, "final", "EP_env_params_{}.pkl".format(i)), "wb"
        ) as fp:
